...
#SBATCH --job-name=fmriprep_study-harrypotter_sub-06_ses-*.job
#SBATCH --output=/lustre03/project/6003287/datasets/cneuromod_processed/fmriprep/harrypotter/code/fmriprep_study-harrypotter_sub-06_ses-*.out
#SBATCH --error=/lustre03/project/6003287/datasets/cneuromod_processed/fmriprep/harrypotter/code/fmriprep_study-harrypotter_sub-06_ses-*.err
...
datalad containers-run -m 'fMRIPrep_sub-06_ses-*' -n containers/bids-fmriprep --input 'sourcedata/*/sub-06/ses-*/{func,fmap}/*_{sbref,bold,epi}.nii.gz' --input 'sourcedata/*/sub-06/{func,fmap}/*_{sbref,bold,epi}.nii.gz' --input 'sourcedata/smriprep/sub-06/anat/' --input 'sourcedata/smriprep/sourcedata/freesurfer/sub-06/' --input 'sourcedata/templateflow/tpl-{MNI152NLin2009cAsym}' --output . -- -w ./workdir --participant-label 06 --anat-derivatives ./sourcedata/smriprep --fs-subjects-dir ./sourcedata/smriprep/sourcedata/freesurfer --bids-filter-file code/fmriprep_study-harrypotter_sub-06_ses-*_bids_filters.json --output-layout bids --ignore slicetiming --use-syn-sdc --output-spaces MNI152NLin2009cAsym T1w:res-iso2mm --cifti-output 91k --notrack --write-graph --skip_bids_validation --omp-nthreads 8 --nprocs 16 --mem_mb 65536 --fs-license-file code/freesurfer.license --resource-monitor sourcedata/harrypotter ./ participant 
fmriprep_exitcode=$?
cp $SLURM_TMPDIR/fmriprep_wf/resource_monitor.json /scratch/bpinsard/fmriprep_study-harrypotter_sub-06_ses-*_resource_monitor.json 
if [ $fmriprep_exitcode -ne 0 ] ; then cp -R $SLURM_TMPDIR /scratch/bpinsard/fmriprep_study-harrypotter_sub-06_ses-*.workdir ; fi 
